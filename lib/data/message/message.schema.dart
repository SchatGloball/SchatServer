...
class _MessageRepository extends BaseRepository
  ...
  Future<List<int>> insert(List<MessageInsertRequest> requests) async {
    ...
    var rows = await db.execute(
      Sql.named(
          'INSERT INTO "messages" ( "body", "forwarded", "author_id", "content", "author_name", "sticker_content", "date_message", "delivered", "original_author", "original_date", "chat_id" )\n'
          'VALUES ${requests.map((r) => '( ${values.add(r.body)}:text, ${values.add(r.forwarded)}:boolean, ${values.add(r.authorId)}:int8, ${values.add(r.content)}:_text, ${values.add(r.authorName)}:text, ${values.add(r.stickerContent)}:int8, ${values.add(r.dateMessage)}:timestamp, ${values.add(r.delivered)}:boolean, ${values.add(r.originalAuthor)}:text, ${values.add(r.originalDate)}:timestamp, ${values.add(r.chatId)}:int8 )').join(', ')}\n'
          'RETURNING "id"'),
  ...
  Future<void> update(List<MessageUpdateRequest> requests) async {
    ...
    await db.execute(
      Sql.named('UPDATE "messages"\n'
          'SET "body" = COALESCE(UPDATED."body", "messages"."body"), "forwarded" = COALESCE(UPDATED."forwarded", "messages"."forwarded"), "author_id" = COALESCE(UPDATED."author_id", "messages"."author_id"), "content" = COALESCE(UPDATED."content", "messages"."content"), "author_name" = COALESCE(UPDATED."author_name", "messages"."author_name"), "sticker_content" = COALESCE(UPDATED."sticker_content", "messages"."sticker_content"), "date_message" = COALESCE(UPDATED."date_message", "messages"."date_message"), "delivered" = COALESCE(UPDATED."delivered", "messages"."delivered"), "original_author" = COALESCE(UPDATED."original_author", "messages"."original_author"), "original_date" = COALESCE(UPDATED."original_date", "messages"."original_date"), "chat_id" = COALESCE(UPDATED."chat_id", "messages"."chat_id")\n'
          'FROM ( VALUES ${requests.map((r) => '( ${values.add(r.id)}:int8::int8, ${values.add(r.body)}:text::text, ${values.add(r.forwarded)}:boolean::boolean, ${values.add(r.authorId)}:int8::int8, ${values.add(r.content)}:_text::_text, ${values.add(r.authorName)}:text::text, ${values.add(r.stickerContent)}:int8::int8, ${values.add(r.dateMessage)}:timestamp::timestamp, ${values.add(r.delivered)}:boolean::boolean, ${values.add(r.originalAuthor)}:text::text, ${values.add(r.originalDate)}:timestamp::timestamp, ${values.add(r.chatId)}:int8::int8 )').join(', ')} )\n'
          'AS UPDATED("id", "body", "forwarded", "author_id", "content", "author_name", "sticker_content", "date_message", "delivered", "original_author", "original_date", "chat_id")\n'
